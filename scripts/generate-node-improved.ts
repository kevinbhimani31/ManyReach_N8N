...
interface FieldConfig {
  displayName: string;
  name: string;
  apiName?: string; // API field name (if different from name)
...
function getValidationHelper(idType?: string): { extract: string; ensure: string } {
  if (idType === 'guid') {
    return { extract: 'extractStringId', ensure: 'ensureGuid' };
  } else if (idType === 'number') {
    return { extract: 'extractNumericId', ensure: 'ensureId' };
  }
...
function generateOperationFile(
  ...
  if (operation.hasPagination || needsValidation) {
    ...
    if (needsValidation) {
      ...
      if (validation.ensure) imports.push(validation.ensure);
    }
    content += imports.join(', ');
    content += ` } from '../../helpers/validation';\n`;
  }
  content += `\n`;
  ...
  if (['getById', 'update', 'delete'].includes(operation.value)) {
    ...
    if (validation.ensure) {
      content += `  ${validation.ensure}(id);\n`;
    }
  }
      ...
      operation.fields.forEach(field => {
        ...
        if (operation.value === 'create' && field.required) {
          if (field.type === 'string') {
            ...
            content += `  body.${apiFieldName} = ${field.name};\n`;
          }
        } else {
  ...
  if (operation.value === 'getAll') {
    ...
    content += `  const response = await apiRequest.call(this, '${operation.method}', \`${operation.endpoint.replace('{id}', '${id}')}\`);\n`;
    content += `\n`;
    content += `  if (!response) {\n`;
    content += `    throw new Error(\`${ResourceName} with ID \${id} not found\`);\n`;
    content += `  }\n`;
    content += `\n`;
    ...
    const endpoint = operation.value === 'update' 
      ? operation.endpoint.replace('{id}', '${id}')
      : operation.endpoint;
...
function generateLoadFile(resource: ResourceConfig, outputDir: string): void {
  ...
  content += `import { loadDropdown, searchResourceLocator } from '../../../helpers/searchHelper';\n`;
  content += `import { apiRequest } from '../../../helpers/apiRequest';\n`;
  ...
  if (resource.hasLoadOptions) {
    content += `export async function load${ResourceName}sForDropdown(\n`;
    content += `  this: ILoadOptionsFunctions,\n`;
    content += `) {\n`;
    content += `  return loadDropdown.call(this, fetch${ResourceName}s);\n`;
    content += `}\n\n`;
  }
  ...
  if (resource.hasListSearch) {
    ...
    content += `) {\n`;
    content += `  return searchResourceLocator.call(this, fetch${ResourceName}s, filter);\n`;
    content += `}\n`;
  }
